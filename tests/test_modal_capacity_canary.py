...
class ModalCapacityCanaryTests(unittest.TestCase):
    def test_fixed_single_l40s_canary_exercises_required_sequence(self):
        ...
        self.assertIn("Qwen/Qwen3-4B", source)
        self.assertIn("1cfa9a7208912126459214e8b04321603b3df60c", source)
        self.assertIn("novaskyai/skyrl-train-ray-2.51.1-py3.12-cu12.8", source)
        for package in ("torch==2.10.0", "vllm==0.19.0", "transformers==5.3.0",
                        "peft==0.18.1", "modal==1.5.5"):
            self.assertIn(package, source)
        self.assertIn("LLM(", source)
        self.assertIn("llm.sleep(level=1)", source)
        self.assertIn("loss.backward()", source)
        self.assertIn("optimizer.step()", source)
        self.assertIn("llm.wake_up()", source)
        self.assertGreaterEqual(source.count("llm.generate("), 2)
        self.assertNotIn("Trial.create", source)
        self.assertNotIn("Daytona", source)
