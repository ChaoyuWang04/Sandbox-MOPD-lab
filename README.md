...
独立的容器 Agentic RL 与多教师在线蒸馏实验。M0执行中：G1实测12288输入/4096输出30.75秒及工具调用通过；G2授权8并发降级和G3正常阶段链路通过，测试进程/沙箱均已回收。G4从零20分钟仍待冷复测，不进入M1。
...
用户已排除 RunPod；本Lab优先home-5090模型推理与调度、Daytona CPU沙箱、Modal有界对照与云训练候选。遵循HOME-5090手册，Mac不跑长期服务。普通prepare/probe已接入并验收，冷prepare入口待注册实跑；终态与证据见EXPERIMENTS。未运行训练或故障注入。
