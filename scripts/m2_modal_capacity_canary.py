"""One-shot Qwen3-4B inference/sleep/LoRA-step/wake capacity canary."""
...
image = (
    ...
    .uv_pip_install("torch==2.10.0", "vllm==0.19.0", "transformers==5.3.0",
...
def capacity_canary() -> str:
    ...
    with authorization.open("x", encoding="utf-8") as handle:
        json.dump({"schema_version": 1, "max_runs": 1, "gpu_type": "L40S",
                   ...
                   "model_revision": MODEL_REVISION}, handle, sort_keys=True)
        handle.write("\n")
        handle.flush()
    ...
    try:
        ...
        pre_digest = adapter_digest(policy)
        ...
        optimizer.step()
        post_digest = adapter_digest(policy)
