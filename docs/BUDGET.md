...
home5090的`m1-import-v2`静态入口尚未登记，已通过真实任务消息协调，未绕过hlab。用户已允许home5090/Modal/Daytona并要求持续完成准备；若入口继续不可用，采用独立Modal Function，仅下载公开资产和本地目录转换，不运行GPU/模型或题目代码。拟用独立Volume `sandbox-mopd-lab-m1-data`，与主线Volume无关；单写者、最多1容器、1物理核、最多2GiB内存、总时限1200秒、无自动重试、镜像构建时限600秒，保存逐资产状态和哈希，Volume数据预留14GiB。Mac只提交代码与读取小结果，不下载模型/镜像或启动服务。
