...
Mac新增资源仍限短时测试/控制，目标RSS不超过0.5GiB，不启模型、容器或常驻服务。home-5090只使用一张现有RTX 5090，GPU recipe必须`exclusive_gpu=true`，先做不创建Daytona的capacity canary；资源不足即停止且不处理他人进程。M2-A Daytona并发硬上限8，实际首跑按2–4题和冻结group size计算创建上限；精确数量及费用必须在`configs/m2-bringup.json`完成后登记，当前未分配新云资源。Modal不作为默认训练或沙箱路径，只有home-5090/Daytona确证阻塞且另行登记时才使用。
