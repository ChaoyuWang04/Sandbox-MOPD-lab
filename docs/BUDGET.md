...
控制器放Modal CPU，1核/最多2GiB，单批最长3900秒，独立结果路径`artifacts/m1/v2/controls/<run-id>/`；Daytona钥匙仅进入控制器秘密配置，不进入题目容器或镜像。总预留含原CPU下载$2后为$10，非账户硬限额。超过此首批的400次全池计划在首批实测之后登记，不无界重试。每次创建前先落盘attempt，错误或超时先查精确label/ID；清理未确认不得继续创建。只回收本批对象，无自动快照常驻；服务端TTL和外层超时均须真实生效。当前只是预算登记，控制器代码和真实六次验收仍未完成。
