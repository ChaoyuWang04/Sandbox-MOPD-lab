...
Daytona探测实际在创建阶段被提供商拒绝，约12秒结束；未获得GPU、未下载或训练，实际账单尚未核对，也不按10分钟上界冒充已消费。只允许一次无create的精确标签清理续跑。回退探测冻结为Modal单张L40S、2 CPU、8GiB内存、最长300秒、并发1、重试0、Lab专用Volume；按2026-09-09官方L40S单价$0.000542/秒，GPU费用硬上界$0.1626，另加CPU/RAM小额。提交前必须确认同名App无活动任务；该探测只验CUDA和持久卷，不下载模型、不启动SkyRL，成功也不等于容量canary或M2-A训练通过。
