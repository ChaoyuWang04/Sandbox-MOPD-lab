...
先运行自建1题、Smith1题、Gym1题各NOP/oracle，共6次，串行；来源内选配置画像明确的轻任务，执行前将精确ID和产物SHA写入独立campaign，不复用旧campaign。每次最多4vCPU/8GiB、10GiB可写磁盘、最长60分钟包含准备与验证，零GPU；参考解与测试需要出站网络，不注入任何业务密钥。Daytona按2026-09-08公开费率CPU $0.0504/vCPU小时、RAM $0.0162/GiB小时、存储 $0.000108/GiB小时，不扣免费额，单次资源上界估计$0.342，6次约$2.052；预留$8包含准备/控制器余量。余额及抵扣未核实，不声称免费；提供商镜像构建费用/配额仍以实际响应记录。
