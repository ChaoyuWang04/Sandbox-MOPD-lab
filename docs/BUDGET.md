...
文档本批未分配GPU/沙箱或下载大资产。200题逐题正负对照名义至少400次trial，另有pilot、基线和train/dev采样；这不是400次已经获准创建。先清点各源CPU/RAM/镜像大小/构建和测试时限/网络需求，再核对当期费率、credits、并发配额与缓存重复计费，给出有界批次估算。原1GiB/5分钟微任务默认不能套用SWE/TB；不满足资源条件的候选在冻结前登记处置，不能随意削弱测试。
