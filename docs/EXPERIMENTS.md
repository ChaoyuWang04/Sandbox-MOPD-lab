...
官方SkyRL v0.2.0源码调查后，严格训练轨迹选择`/chat/completions+return_token_ids`逐轮step-wise路线：generated token、逐token rollout logprob、position、mask和权重版本必须对齐；M1文本trace及上游HarborGenerator的事后重tokenize输出不作训练输入。M2冻结单张专用云GPU上的colocate/local-engine/sleep-wake拓扑，先做Daytona RTX 5090→4090一次性能力探测，再做不创建任务沙箱的capacity canary；Daytona不成立才冻结Modal回退。final100改为训练/晋级终止后Base+唯一Candidate的一次性同批解封，任何结果可见后本研究不再训练。EvoCodeBench10保持独立eval-only，未下载、未加入200题。本节是跑前契约，不是M1/M2通过或资源已分配。

首个源码锁固定官方`skyrl-v0.2.0@eddb418dd4c560db9d43ffde561f1c5e669c8990`，其`pyproject.toml`/`uv.lock` SHA256分别为`f7c5813b8fc0efd667288b43ee020cde78773d35d7e57c315f674954fc7f421b`/`6919d985d0365ef4a660fb60718355bdfb9bdecaacd13a198157333e68a6bc74`；锁定Harbor `0.4.0@a85628c803dc1713fcd26d2bb1908cde69d62317`、Python3.12.12、torch2.10.0+cu128、vLLM0.19.0、Ray2.51.1、Transformers5.3.0、Modal1.4.1。2026-09-09用户改定M2训练不使用home-5090，目标顺序为Daytona GPU→Modal回退；Daytona拒绝后，当前锁选择Modal/L40S，仍保留Harbor 0.4任务兼容、完整SkyRL capacity canary和严格step-wise bridge三个未验项，所以`configs/m2-stack-lock.json`明确`execution_ready=false`。这些仍只是能力探测与离线配置，不是M2-A训练通过。Lab外层控制器继续使用自身Modal1.5.5；它与SkyRL训练venv的1.4.1是两个明确环境，不能混写成同一runtime。
...
清理续跑使用源码`faea84dffdf524d2a3a529981dbfc473d9fd9537`、plan `plan-sandbox-rl-mopd-20260909t044102z-82d010a3`、run `run-sandbox-rl-mopd-20260909t044118z-c9e2bc1c`，7秒成功：stdout为`cleanup_confirmed_no_retry=true`且`matched_sandbox_ids=[]`，证明没有再次create，也没有本run归属对象残留。Modal App `ap-QyoWdSR0v7NW5lUMUO0CZM`随后完成一次L40S远端探测并停止、tasks=0；专用Volume `sandbox-mopd-lab-m2`的`probes/modal-gpu-v1.json` SHA256为`eccf26ea2349bfb8dad7c342b16745c4a30dd6e1f921543f98c37ed9eeaa570f`，记录单张NVIDIA L40S、47665709056字节显存、CUDA12.8、torch2.10.0+cu128、Python3.12.3和Volume可写。远端返回值最初因`torch.__version__`保留PyTorch字符串子类而在无torch的Mac反序列化失败；Volume中的纯JSON已先写成`phase=complete`，因此未重跑GPU，只修正后续返回为内建`str`。此探测证明Modal GPU与持久卷可用，不证明Qwen3-4B+SkyRL完整栈容量足够。
...
为避免用GPU发现简单依赖缺口，CPU-only App `ap-g4KROwURhyoXoOkVkXodGD`先检查官方`novaskyai/skyrl-train-ray-2.51.1-py3.12-cu12.8`镜像：它只预装Ray2.51.1，系统Python中torch/vLLM/Transformers/PEFT/Modal/Harbor包均未安装，终态按预注册规则记`mismatch`。这不是CUDA或训练失败，而是证明官方示例依赖`uv run`在挂载源码上另建环境。第二个CPU-only App `ap-TrZzCMlvSnkHSf7vv0W3kx`尝试用普通pip叠加精确版本，镜像构建在分配函数资源前被解析器拒绝：vLLM0.19声明Transformers `<5`，而SkyRL锁定Transformers5.3。逐字核对固定提交后确认这不是随意降级即可解决的冲突：SkyRL的`pyproject.toml`在`tool.uv.override-dependencies`明确覆盖该上游声明，并由`uv.lock`、专用torch/flashinfer索引和flash-attn wheel共同冻结。因此第三版CPU预检和capacity canary都改为在固定SkyRL提交上执行`uv sync --frozen --extra fsdp --extra harbor --no-dev`，不使用`--no-deps`或自行改版本。

capacity canary的固定序列仍为Qwen3-4B推理→vLLM level-1 sleep→单步LoRA反向/更新/保存→wake→再次推理，不创建Harbor任务沙箱。它通过后仍需独立验证SkyRL/Harbor真实入口，不能单凭直接组件canary清空全部未验项；且只有M1 train-screen按冻结规则通过后才允许提交GPU canary。
